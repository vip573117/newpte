...
if ($operation == 'display') {
    ...
    $psize = 5;
    //使用拼接sql语句
    $sql = "SELECT * FROM " . tablename('pte_examine') . " where status!=-1  and type=1";
    $sql .= " limit " . ($pindex - 1) * $psize . ',' . $psize;
    $examine = pdo_fetchall($sql);
    $total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('pte_examine') . " where status!=-1 and type=1");
    //（前台用pager标签可以直接显示）
    $pager = pagination($total, $pindex, $psize);
    $teacher = pdo_fetchall("select * from " . tablename('pte_teacher') . " where status=1");
    //var_dump( $teacher);
    include $this->template('admin/approval');
}
if ($operation == 'status') {
    $id = $_GPC['id'];
    $type = $_GPC['type'];
    ...
    if ($_W['isajax'] && $_W['ispost']) {
        ...
        $teacher = pdo_fetch("select * from " . tablename('pte_examine') . " where type=1 and teacher_id=".  $teacher_id);
        if ( $teacher) {
